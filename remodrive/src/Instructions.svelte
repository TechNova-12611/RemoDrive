<div>
    <h1>Keybinds</h1>

    <p class="lead">RemoDrive allows simulating a gamepad through the keyboard. If you have a gamepad, plug it in and toggle the switch in the top-right to use it. These keybinds are based on the gamepad below.</p>
    <img src="/static/gamepad.png" alt="Gamepad"/>

    <br>
    <h2>Buttons</h2>

    <table class="table table-striped">
        <thead>
            <tr>
                <th scope="col">Action</th>
                <th scope="col">Key</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>A Key</td>
                <td>Z</td>
            </tr>
            <tr>
                <td>B Key</td>
                <td>X</td>
            </tr>
            <tr>
                <td>X Key</td>
                <td>C</td>
            </tr>
            <tr>
                <td>Y Key</td>
                <td>V</td>
            </tr>
            <tr>
                <td>Right Trigger</td>
                <td>Period</td>
            </tr>
            <tr>
                <td>Left Trigger</td>
                <td>Comma</td>
            </tr>
            <tr>
                <td>Right Bumper</td>
                <td>Apostrophe</td>
            </tr>
            <tr>
                <td>Left Bumper</td>
                <td>Semicolon</td>
            </tr>
            <tr>
                <td>D-Pad Up</td>
                <td>I</td>
            </tr>
            <tr>
                <td>D-Pad Left</td>
                <td>J</td>
            </tr>
            <tr>
                <td>D-Pad Down</td>
                <td>K</td>
            </tr>
            <tr>
                <td>D-Pad Right</td>
                <td>L</td>
            </tr>
        </tbody>
    </table>

    <br>
    <h2>Joysticks</h2>

    <table class="table table-striped">
        <thead>
            <tr>
                <th scope="col">Action</th>
                <th scope="col">Key</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>Left Joystick Up</td>
                <td>W</td>
            </tr>
            <tr>
                <td>Left Joystick Left</td>
                <td>A</td>
            </tr>
            <tr>
                <td>Left Joystick Down</td>
                <td>S</td>
            </tr>
            <tr>
                <td>Left Joystick Right</td>
                <td>D</td>
            </tr>

            <tr>
                <td>Right Joystick Up</td>
                <td>Up Arrow</td>
            </tr>
            <tr>
                <td>Right Joystick Left</td>
                <td>Left Arrow</td>
            </tr>
            <tr>
                <td>Right Joystick Down</td>
                <td>Down Arrow</td>
            </tr>
            <tr>
                <td>Right Joystick Right</td>
                <td>Right Arrow</td>
            </tr>
        </tbody>
    </table>
</div>